...
public class ClusterNodeInformationParser {
  ...
  public static final int SLOT_INFORMATIONS_IDENTITY = 0;
  public static final int SLOT_INFORMATIONS_MASTER_IDENTITY = 3;
  ...
  public HostAndPort getHostAndPortFromNodeLine(String[] nodeInfoPartArray, HostAndPort current) {
	String stringIdentity = nodeInfoPartArray[SLOT_INFORMATIONS_IDENTITY];
    String stringHostAndPort = nodeInfoPartArray[HOST_AND_PORT_INDEX];

    String[] arrayHostAndPort = stringHostAndPort.split(":");
    return new HostAndPort(stringIdentity,arrayHostAndPort[0].isEmpty() ? current.getHost() : arrayHostAndPort[0],
