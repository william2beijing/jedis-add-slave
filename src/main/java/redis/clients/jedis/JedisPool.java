...
public class JedisPool extends Pool<Jedis> {
  ...
  public JedisPool(final String host) {
    ...
    if (JedisURIHelper.isValid(uri)) {
      ...
      this.internalPool = new GenericObjectPool<Jedis>(new JedisFactory(h, port,
          Protocol.DEFAULT_TIMEOUT, Protocol.DEFAULT_TIMEOUT, password, database, null, false),
          new GenericObjectPoolConfig());
    } else {
      this.internalPool = new GenericObjectPool<Jedis>(new JedisFactory(host,
          Protocol.DEFAULT_PORT, Protocol.DEFAULT_TIMEOUT, Protocol.DEFAULT_TIMEOUT, null,
          Protocol.DEFAULT_DATABASE, null, false), new GenericObjectPoolConfig());
    }
  }
  ...
  public JedisPool(final GenericObjectPoolConfig poolConfig, final String host, int port,
      ...
      final String clientName, final boolean isReadOnly) {
    super(poolConfig, new JedisFactory(host, port, connectionTimeout, soTimeout, password,
        database, clientName, isReadOnly));
  }
